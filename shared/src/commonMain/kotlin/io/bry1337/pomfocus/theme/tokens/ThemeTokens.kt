package io.bry1337.pomfocus.theme.tokens

/**
 * Created by Bryan on 2/25/23.
 * Copyright (c) 2023 bry1337.github.io. All rights reserved.
 */

object ThemeTokens {
    // Interaction States
    // https://m3.material.io/foundations/interaction-states
    //
    // Scaled up the values to work with our app colors
    const val DisabledContainerOpacity = 0.12 * 2.0
    const val DisabledContentOpacity = 0.38 * 1.5
    const val MutedContentOpacity = 0.40
    const val SlightlyMutedContentOpacity = 0.60
    const val FadedBackgroundOpacity = 0.60
    const val BackgroundForTextColorOpacity = 0.10

    const val DividerThickness = 1.0

    const val gradientBackgroundOpacity = 0.75
    const val gradientBackgroundSizeScale = 0.9f
}
